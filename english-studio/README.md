...
- **TTS** — 기본은 로컬 Kokoro-82M:
  ```sh
  docker run -p 8880:8880 ghcr.io/remsky/kokoro-fastapi-cpu:latest
  ```
  OpenAI(`OPENAI_API_KEY`) / ElevenLabs(`ELEVENLABS_API_KEY`)로 전환 가능.
- **STT** — 기본은 로컬 MLX VibeVoice(`http://localhost:8000`). 서버가 없으면
  자동으로 브라우저 WebSpeech로 폴백. `whisper` 선택 시 OpenAI 키 필요.
- **문법 검사(LanguageTool)** — 기본은 로컬 서버:
  ```sh
  docker run -p 8010:8010 erikvl87/languagetool
  ```
  로컬 서버 없이 쓰려면 `writing.html` 상단 드롭다운에서 **Public API**로
  바꾸면 된다 (무료 공개 엔드포인트, 요청 빈도 제한 있음, 텍스트가
  languagetool.org로 전송됨). 기본 선택값은 `config.local.js`의
